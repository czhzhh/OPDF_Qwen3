...
def _extract_loss_and_logits(outputs):
    ...
    if loss is None or logits is None:
        if isinstance(outputs, (tuple, list)):
            if loss is None and len(outputs) >= 1:
                loss = outputs[0]
            if logits is None and len(outputs) >= 2:
                logits = outputs[1]
    if loss is None or logits is None:
        raise ValueError("Model outputs must contain loss and logits for LGTM distillation.")
    return loss, logits

def _flatten_logits(logits):
    if logits is None:
        return None
    if logits.dim() > 2:
...
def cal_loss(s_logits, t_logits, temperature):
    t_logits = _flatten_logits(t_logits)
    s_logits = _flatten_logits(s_logits)
...
class LGTMTeacher(object):
    ...
    def cal_stu_tea_loss(self, teacher_outputs, student_outputs, flag=1):
        t_loss, t_logits = _extract_loss_and_logits(teacher_outputs)
        loss, logits = _extract_loss_and_logits(student_outputs)
    ...
    def _backward_step_unrolled(self, inputs, eval_inputs, network_optimizer):
        ...
        for k, v in eval_inputs.items():
            eval_inputs[k] = v.to(_model_device(unrolled_model))
        unrolled_model.train()
        outputs = unrolled_model(**eval_inputs)
        unrolled_loss, _ = _extract_loss_and_logits(outputs)
    ...
    def _compute_unrolled_model(self, input, network_optimizer):
        for k, v in input.items():
            input[k] = v.to(_model_device(self.teacher_model))
    ...
    def _construct_model_from_theta(self, theta):
        ...
        return model_new.to(_model_device(self.student_model))

    def _hessian_vector_product(self, vector, input, r=1e-2):
        norm = _concat(vector).norm()
        if norm == 0:
            return [torch.zeros_like(p) for p in self.teacher_model.parameters()]
        ...
        return [(x - y).div_(2 * R + 1e-9) for x, y in zip(grads_p, grads_n)]
